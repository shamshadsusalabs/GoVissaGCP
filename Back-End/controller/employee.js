...
exports.getUserVisaDetails = async (req, res) => {
  try {
    ...
    console.log(`Employee ${userId} (${user.name}) has:`);
    console.log(`- applicationIds: [${applicationIds.join(', ')}] (${applicationIds.length} applications)`);
    console.log(`- visaIds: [${visaIds.join(', ')}] (${visaIds.length} visaIds)`);

    // If we have applicationIds, use them directly to fetch individual applications
    if (applicationIds.length > 0) {
      console.log('Using new applicationIds system...');
      ...
      const applicationObjectIds = applicationIds.map(id => {
        try {
          ...
          console.log(`Invalid ApplicationId ObjectId: ${id}`);
          return null;
        }
      }).filter(id => id !== null);
      ...
      console.log(`Found ${visaApplications.length} visa applications by applicationIds`);
    ...
    console.log('Falling back to old visaIds system...');
    ...
    console.log(`Searching for visaIds:`, visaIds);
    console.log(`Query: { visaId: { $in: [${visaIds.join(', ')}] } }`);
    ...
    // Also check what visa documents exist in the database
    const totalVisas = await Visa.countDocuments();
    const sampleVisas = await Visa.find().limit(2).select('_id visaId').lean();
    
    console.log(`Employee ${userId} has ${visaIds.length} visa IDs`);
    console.log(`Found ${visasByObjectId.length} visa documents by _id`);
    console.log(`Found ${visasByVisaId.length} visa documents by visaId`);
    console.log(`Found ${visasByEmail.length} visa documents by email (${user.email})`);
    console.log(`Found ${visasByPartialEmail.length} visa documents by partial/common email matching`);
    console.log(`Total visa documents in database: ${totalVisas}`);
    console.log(`Sample visa IDs from employee:`, visaIds.slice(0, 2));
    console.log(`Sample visa documents from database:`, sampleVisas);

    // Debug: Let's see what visaIds are actually in the returned documents
    console.log(`First 5 returned visa documents:`, visasByVisaId.slice(0, 5).map(v => ({ _id: v._id, visaId: v.visaId })));
    ...
    console.log(`After filtering: found ${filteredVisasByVisaId.length} visa documents (including duplicates)`);
    console.log(`After removing duplicates: found ${uniqueVisas.length} unique visa documents`);
    console.log(`Unique visa IDs:`, uniqueVisas.map(v => v.visaId));
    
    // Only use visaIds array - no email-based matching
    let visas = [];
    if (visasByObjectId.length > 0) {
      visas = visasByObjectId;
      console.log(`Using ObjectId-based matching - found ${visas.length} visas`);
    } else if (uniqueVisas.length > 0) {
      visas = uniqueVisas;
      console.log(`Using unique visaId-based matching - found ${visas.length} visas`);
    } else {
      console.log(`No visa applications found for employee's visaIds array`);
    }
    ...
    if (visas.length > 0 && visas.length < visaIds.length) {
      ...
      console.log(`Cleaned up employee visaIds: removed ${visaIds.length - updatedVisaIds.length} non-existent visa IDs`);
    }
...
exports.migrateVisaIdsToApplicationIds = async (req, res) => {
  try {
    console.log('Starting migration from visaIds to applicationIds...');
    ...
    for (const employee of employees) {
      ...
      console.log(`   Current visaIds: [${employee.visaIds.join(', ')}]`);
      ...
      for (const visaId of employee.visaIds) {
        ...
        console.log(`   VisaId ${visaId}: found ${applications.length} applications`);
        
        for (const app of applications) {
          applicationIds.push(app._id.toString());
          console.log(`     - Added application ${app._id} (${app.country})`);
        }
      }
      ...
      if (applicationIds.length > 0) {
        ...
        console.log(`   Migrated ${employee.applicationIds.length} applications for ${employee.name}`);
        migratedCount++;
        totalApplicationsAssigned += employee.applicationIds.length;
      } else {
        console.log(`   No applications found for ${employee.name}'s visaIds`);
      }
    }

    console.log(`Migration completed!`);
    console.log(`   - Employees migrated: ${migratedCount}`);
    console.log(`   - Total applications assigned: ${totalApplicationsAssigned}`);
