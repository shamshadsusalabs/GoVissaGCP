...
const createVisaApplication = async (req, res) => {
  try {
    ...
    if (paymentId) {
      try {
        ...
        if (paymentOrder) {
          ...
          console.log("🎫 Promo Code Data Retrieved from Payment Order:", promoCodeData);
          console.log("🆔 Payment Order MongoDB ID:", paymentOrderMongoId);
        } else {
          console.log("⚠️ Payment order not found for paymentId:", paymentId);
        }
      } catch (error) {
    ...
    // ✅ Console log promo code data received (fallback to request body if not found in payment order)
    ...
    console.log("promoCode:", finalPromoCode)
    console.log("promoCodeId:", finalPromoCodeId)
    console.log("discountAmount:", finalDiscountAmount)
    console.log("originalAmount:", finalOriginalAmount)
    console.log("paymentOrderId (MongoDB _id):", paymentOrderMongoId || paymentOrderId)
    console.log("paymentId (Razorpay):", paymentId)
    ...
    console.log("✅ Visa Application Saved with Processing Info:", {
      ...
      promoCode: savedVisaApplication.promoCode,
      promoCodeId: savedVisaApplication.promoCodeId,
      discountAmount: savedVisaApplication.discountAmount,
      originalAmount: savedVisaApplication.originalAmount,
      paymentOrderId: savedVisaApplication.paymentOrderId,
    })
