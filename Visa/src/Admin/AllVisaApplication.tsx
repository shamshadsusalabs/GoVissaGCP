...
const AllVisaApplication: React.FC = () => {
  ...
  const fetchVisaConfigurations = async () => {
    try {
      ...
      console.error("Error fetching visa configurations:", err)
    }
  }
  ...
  const fetchPaymentOrders = async () => {
    try {
      ...
      if (data.success && data.data) {
        ...
        console.log("✅ Payment orders loaded:", data.data.length, "orders")
      }
    } catch (err) {
  ...
  const getTravellerDetails = (paymentOrderId: string) => {
    console.log("🔍 Looking for payment order:", paymentOrderId)
    console.log("📋 Available payment orders:", paymentOrders.map(o => ({ _id: o._id, travellerDetails: o.travellerDetails })))
    const paymentOrder = paymentOrders.find(order => order._id === paymentOrderId)
    console.log("💳 Found payment order:", paymentOrder ? "Yes" : "No")
    if (paymentOrder && paymentOrder.travellerDetails) {
      console.log("👥 Traveller details:", paymentOrder.travellerDetails)
          ...
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {paginatedData.map((app) => {
                ...
                // Debug console logs
                console.log(`🔍 App ${app._id.substring(0, 8)}:`, {
                  appId: app._id,
                  appVisaId: app.visaId,
                  assignedTo: assignedEmployee?.name || 'None',
                  isAssigned: isAssigned,
                  employeeApplicationIds: employees.map(emp => ({
                    name: emp.name,
                    applicationIds: emp.applicationIds || [],
                    visaIds: emp.visaIds || [], // Also show old visaIds for debugging
                    hasAppId: (emp.applicationIds || []).includes(app._id)
                  }))
                })
