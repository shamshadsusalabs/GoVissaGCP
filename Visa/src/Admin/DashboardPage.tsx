...
const DashboardPage: React.FC = () => {
  ...
  const fetchPendingPayments = async () => {
    try {
      ...
      if (data.success) {
        ...
        console.log("✅ Set pending payments:", data.data.length)
      } else {
        console.error("❌ Failed to fetch pending payments:", data.message)
      }
    } catch (err) {
  ...
  // Open payment approval dialog
  const openPaymentApprovalModal = (payment: any) => {
    console.log("🔓 Opening payment approval modal for:", payment)
    ...
    console.log("✅ Payment approval dialog should be open now")
  }
  ...
  const getVisaTypeFromId = (visaId: string) => {
    console.log("🔍 Looking for visaId:", visaId)
    console.log("🔍 Available visa configurations:", visaConfigurations.map(c => ({ id: c._id, country: c.countryDetails?.name })))
    
    // Dynamic mapping using actual visa configurations from API
    const visaConfig = visaConfigurations.find(config => config._id === visaId)
    if (visaConfig) {
      console.log("✅ Found visa config:", visaConfig)
      ...
      if (visaConfig.visaTypes && visaConfig.visaTypes.length > 0) {
        ...
        console.log("📋 Returning visa type from visaTypes:", visaType)
        return visaType
      }
    ...
    console.log("❌ No visa config found, returning Tourist")
