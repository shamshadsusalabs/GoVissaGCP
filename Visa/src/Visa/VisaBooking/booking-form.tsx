...
const BookingForm: React.FC<BookingFormProps> = ({
  ...
  const handlePayment = async () => {
    ...
    try {
      // Fixed calculation: Both government fee and service fee should be multiplied by travelers
      const originalAmount = (selectedVisaType.visaFee + selectedVisaType.serviceFee) * travellers
    ...
    try {
      ...
      const response = await fetch("http://localhost:5000/api/payments/create-cash-order", {
        ...
        body: JSON.stringify({
          ...
          discountAmount: discountAmount,
          cashDetails: {
            notes: `Cash payment for ${visaData.country} visa - ${travellers} traveller(s)`
          }
        }),
      })
    ...
    try {
      ...
      const response = await fetch("http://localhost:5000/api/payments/create-paylater-order", {
        ...
        body: JSON.stringify({
          ...
          payLaterDetails: {
            ...payLaterDetails,
            creditLimit: amount / 100 // Convert back to rupees
          }
        }),
      })
  ...
  // Calculate total per traveler first, then multiply by number of travelers
