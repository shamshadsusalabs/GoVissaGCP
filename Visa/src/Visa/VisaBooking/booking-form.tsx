...
const BookingForm: React.FC<BookingFormProps> = ({
  ...
  const [children, setChildren] = useState(0) // 2y - 12y
  const [infants, setInfants] = useState(0)   // below 2y
        ...
        if (currentUser && currentUser._id && !currentUser.email) {
          try {
            ...
            if (updateResponse.ok) {
              ...
              console.log("✅ Email added to existing user profile")
            }
          } catch (updateErr) {
            console.error("Failed to update user profile with email:", updateErr)
          }
        }
        
        console.log("Email OTP verified successfully:", data.message)
      } else {
  ...
  const handlePayment = async () => {
    ...
    try {
      // Fixed calculation: Both government fee and service fee should be multiplied by total travelers (adults + children + infants)
      const totalTravellers = travellers + children + infants
      const originalAmount = (selectedVisaType.visaFee + selectedVisaType.serviceFee) * totalTravellers
    ...
    try {
      ...
      const response = await fetch("http://localhost:5000/api/payments/create-cash-order", {
        ...
        body: JSON.stringify({
          ...
          originalAmount: subtotal.toString(),
          cashDetails: {
            notes: `Cash payment for ${visaData.country} visa - ${totalTravellers} traveller(s) (Adults: ${travellers}, Children: ${children}, Infants: ${infants})`
          }
        }),
      })
      ...
      const result = await response.json()
      console.log("Cash payment order created:", result)
  ...
  const handlePayLaterPayment = async () => {
    ...
    try {
      ...
      const result = await response.json()
      console.log("Pay Later order created:", result)
      setPaymentSuccess("paylater") // ✅ Pass payment method type for pay later
      setError(null)
    } catch (err) {
      console.error("Error creating pay later order:", err)
  ...
  // Calculate total per traveler first, then multiply by TOTAL number of travelers (adults + children + infants)
  const totalPerTraveler = governmentFee + serviceFee
  const subtotal = totalPerTraveler * totalTravellers
      ...
      return (
          ...
          <div className="p-4 sm:p-6 lg:p-8 space-y-6 sm:space-y-8">
            <div className="border-b border-gray-200 pb-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center p-4 bg-gray-50 rounded-lg">
                  <div>
                    <div className="font-semibold text-gray-800">CHILDREN (2y - 12y)</div>
                    <div className="text-sm text-gray-500">on the day of travel</div>
                  </div>
                  <div className="flex items-center space-x-3">
                    <button
                ...
                <div className="flex justify-between items-center p-4 bg-gray-50 rounded-lg">
                  <div>
                    <div className="font-semibold text-gray-800">INFANTS (below 2y)</div>
                    <div className="text-sm text-gray-500">on the day of travel</div>
                  </div>
                  <div className="flex items-center space-x-3">
                    <button
                    ...
                    <span className="font-bold text-lg w-6 text-center text-gray-900 bg-blue-500 text-white rounded px-2 py-1">{infants}</span>
                    <button
        ...
        <div className="space-y-4 border-t border-gray-200 pt-8">
          ...
          <div className="flex justify-between text-base text-gray-700">
            ...
            <span className="text-green-600 font-bold">₹ {serviceFee.toFixed(2)}</span>
          </div>
          <div className="flex justify-between text-base text-gray-700">
            <span>Total per person</span>
            <span className="font-medium">₹ {totalPerTraveler.toFixed(2)}</span>
          </div>
          <div className="flex justify-between text-base text-gray-700">
