...
// Common documents required for tourist visa
const COMMON_DOCUMENTS = [
  {
    title: "Valid Passport",
    description: "Original passport (valid for at least 6 months beyond your return date)\n\nMinimum 2 blank pages\n\nAttach copies of previous visas (if any)"
  },
  {
    title: "Passport-Sized Photographs",
    description: "Recent photos (usually 35mm x 45mm)\n\nWhite background\n\nAs per specific embassy photo guidelines"
  },
  {
    title: "Proof of Accommodation",
    description: "Hotel bookings or invitation letter from host (if staying with family/friends)\n\nAirbnb confirmation or similar accepted in most cases"
  },
  {
    title: "Proof of Financial Means",
    description: "Bank statements for last 3–6 months (stamped)\n\nIncome tax returns (ITR) or Form 16\n\nPay slips (if employed)\n\nSponsorship letter (if someone else is funding your trip)"
  },
  {
    title: "Travel Insurance",
    description: "Valid for entire duration of stay\n\nMinimum coverage (e.g., €30,000 for Schengen)\n\nMust cover medical emergencies, hospitalization, and repatriation"
  },
  {
    title: "Flight Reservation / Booking",
    description: "Return ticket booking (do NOT purchase until visa is granted unless required)\n\nProof of internal travel (trains/flights between cities if applicable)"
  }
];
...
const VisaRequirements = () => {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-10">
        <div className="flex items-center gap-3 mb-6">
          <div className="h-0.5 w-8 bg-green-500 rounded-full"></div>
          <h3 className="text-xl font-semibold text-gray-800">Common Documents Required for Tourist Visa Application</h3>
          <span className="text-xs bg-green-100 text-green-800 px-2.5 py-1.5 rounded-full font-medium">
            {COMMON_DOCUMENTS.length} documents
          </span>
        </div>
          ...
          {COMMON_DOCUMENTS.map((doc, index) => (
            ...
            >
              <div className="flex items-start gap-4 p-5">
                <div className="flex-shrink-0 text-2xl mt-0.5 bg-green-50 p-3 rounded-lg">
                  {DOCUMENT_ICONS[doc.title] || '📄'}
                </div>
                <div>
